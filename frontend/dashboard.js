...
async function fetchUserProfile() {
    try {
        const response = await fetch("http://localhost:5000/api/v1/user/me", { credentials: "include" });
...
async function fetchOrderHistory() {
    try {
        const response = await fetch("http://localhost:5000/api/v1/user/orders", { credentials: "include" });
        const data = await response.json();
        ...
        if (response.ok) {
            ...
            data.orders.forEach(order => {
                ...
                li.textContent = `Order ID: ${order._id} | Items: ${order.items.map(item => item.menuItem.name).join(", ")} | Total: ${order.totalPrice}`;
...
async function fetchAppointmentHistory() {
    try {
        ...
        if (response.status === 404) {  // Handle "No Appointments Found"
            console.log("No appointment history found.");
            document.getElementById("appointmentsList").innerHTML = "<li>No appointments found.</li>";
            return;
        }
